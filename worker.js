  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/appdata" && request.method === "GET") {
        const record = await readBoard(env);
        return json({
          data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: record.groupRoster || [] },
          role: auth.role,
          email: auth.email,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        await writeBoard(env, record);
        return json({ data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: record.groupRoster || [] } });
      }

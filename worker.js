...
const BIN_ID = "6a9b462bf5f4af5e296bd02d";
...
async function readBoard(env) {
  ...
  // Миграция старого формата: раньше весь бин был просто массивом отчётов
  // (без пользователей вообще — те жили только в localStorage браузера).
  if (Array.isArray(record)) {
    record = { users: [], reports: record };
  }
...
function stripPasswords(users) {
  return (users || []).map((u) => ({ email: u.email, group: u.group, role: u.role }));
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/register" && request.method === "POST") {
        ...
        record.users.push({ email, pass, group, role: "user" });
        await writeBoard(env, record);
        const token = await signToken({ email, role: "user", exp: Date.now() + TOKEN_LIFETIME_MS }, env);
        return json({ token, email, role: "user", group });
      }
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        const token = await signToken({ email: user.email, role: user.role, exp: Date.now() + TOKEN_LIFETIME_MS }, env);
        return json({ token, email: user.email, role: user.role, group: user.group });
      }
      ...
      if (path === "/appdata" && request.method === "GET") {
        const record = await readBoard(env);
        return json({
          data: { reports: record.reports, users: stripPasswords(record.users) },
          role: auth.role,
          email: auth.email,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        await writeBoard(env, record);
        return json({ data: { reports: record.reports, users: stripPasswords(record.users) } });
      }

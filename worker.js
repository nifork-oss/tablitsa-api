...
function stripPasswords(users) {
  return (users || []).map((u) => ({ email: u.email, group: u.group, role: u.role, approved: u.approved === undefined ? true : !!u.approved, notesAccess: !!u.notesAccess, notesAccessExpiresAt: u.notesAccessExpiresAt || null }));
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        return json({ token, email: user.email, role: user.role, group: user.group, approved: isApproved, notesAccess: hasNotesAccess });
      }
      ...
      if (path === "/appdata" && request.method === "GET") {
        ...
        return json({
          data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: stripRosterNames(record.groupRoster, hasNotesAccess), groupRosterDraft: isSuperadmin ? (record.groupRosterDraft || []) : [], rosterNotes: (hasNotesAccess && isAdmin) ? (record.rosterNotes || {}) : {}, notesAccessRequests: isSuperadmin ? (record.notesAccessRequests || []) : (record.notesAccessRequests || []).filter((r) => r.email === auth.email) },
          role: auth.role,
          email: auth.email,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        if (incoming.rosterNotes && typeof incoming.rosterNotes === "object" && !Array.isArray(incoming.rosterNotes)) {
          ...
          const hasNotesAccessForWrite = isAdmin && computeHasNotesAccess(currentUserRecordForNotes, isSuperadmin);
        ...
        return json({ data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: stripRosterNames(record.groupRoster, hasNotesAccessFinal), groupRosterDraft: isSuperadmin ? (record.groupRosterDraft || []) : [], rosterNotes: (hasNotesAccessFinal && isAdmin) ? (record.rosterNotes || {}) : {}, notesAccessRequests: isSuperadmin ? (record.notesAccessRequests || []) : (record.notesAccessRequests || []).filter((r) => r.email === auth.email) } });
      }

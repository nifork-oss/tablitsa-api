...
async function readBoard(env) {
  ...
  if (!Array.isArray(record.groupRosterDraft)) record.groupRosterDraft = [];
  if (!record.rosterNotes || typeof record.rosterNotes !== "object" || Array.isArray(record.rosterNotes)) record.rosterNotes = {};
  if (!Array.isArray(record.notesAccessRequests)) record.notesAccessRequests = [];
...
function stripPasswords(users) {
  return (users || []).map((u) => ({ email: u.email, group: u.group, role: u.role, approved: u.approved === undefined ? true : !!u.approved, notesAccess: !!u.notesAccess, notesAccessExpiresAt: u.notesAccessExpiresAt || null, notesContentAccess: !!u.notesContentAccess }));
}

function computeHasNotesAccess(userRecord, isSuperadminFlag) {
  if (isSuperadminFlag) return true;
  if (!userRecord) return false;
  if (userRecord.notesAccess === true) return true;
  if (userRecord.notesAccessExpiresAt && Date.now() < userRecord.notesAccessExpiresAt) return true;
  return false;
}

// Доступ к самим ЗАМЕТКАМ (текст с личными сведениями) — отдельное право от
// доступа к именам. Выдаётся только вручную главным админом в настройках
// доступа, никакого запроса/срока действия здесь нет — либо есть, либо нет.
function computeHasNotesContentAccess(userRecord, isSuperadminFlag) {
  if (isSuperadminFlag) return true;
  if (!userRecord) return false;
  return userRecord.notesContentAccess === true;
}

// Список состава (Основной список) — группа/номер/статус можно смотреть всем
// админам (это не тайна), а вот сами ИМЕНА возвещателей — только с доступом,
// точно так же, как заметки. Прячем имя на сервере, а не только в интерфейсе,
// чтобы данные физически не долетали до браузера без права их видеть.
function stripRosterNames(roster, hasAccess) {
  if (hasAccess) return roster || [];
  return (roster || []).map((p) => ({ ...p, name: "", personId: "" }));
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        const hasNotesAccess = computeHasNotesAccess(user, user.role === "superadmin");
        const hasNotesContentAccess = computeHasNotesContentAccess(user, user.role === "superadmin");
        const token = await signToken({ email: user.email, role: user.role, group: user.group, exp: Date.now() + TOKEN_LIFETIME_MS }, env);
        return json({ token, email: user.email, role: user.role, group: user.group, approved: isApproved, notesAccess: hasNotesAccess, notesContentAccess: hasNotesContentAccess });
      }
      ...
      if (path === "/appdata" && request.method === "GET") {
        const record = await readBoard(env);
        // Доступ к заметкам о возвещателях смотрим свежо в базе (не из токена),
        // чтобы отзыв/выдача доступа супер-админом срабатывали немедленно,
        // без необходимости перезаходить в систему.
        const currentUserRecord = record.users.find((u) => u.email === auth.email);
        const hasNotesAccess = computeHasNotesAccess(currentUserRecord, isSuperadmin);
        const hasNotesContentAccess = computeHasNotesContentAccess(currentUserRecord, isSuperadmin);
        return json({
          data: { reports: record.reports, users: isSuperadmin ? stripPasswords(record.users) : [], ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: stripRosterNames(record.groupRoster, hasNotesAccess), groupRosterDraft: isSuperadmin ? (record.groupRosterDraft || []) : [], rosterNotes: hasNotesContentAccess ? (record.rosterNotes || {}) : {}, notesAccessRequests: isSuperadmin ? (record.notesAccessRequests || []) : (record.notesAccessRequests || []).filter((r) => r.email === auth.email) },
          role: auth.role,
          email: auth.email,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        // рабочий процесс приложения — заполнение своих отчётов). Но обычный
        // пользователь (не админ) может менять только записи своей группы —
        // всё остальное берём как было в базе, что бы клиент ни прислал.
        // Это защита от подмены чужих данных, даже если кто-то обойдёт
        // интерфейс и обратится к API напрямую.
        if (Array.isArray(incoming.reports)) {
          if (isAdmin) {
            record.reports = incoming.reports;
          } else {
            const myGroup = String(auth.group || "");
            const othersUnchanged = record.reports.filter((r) => String(r.group) !== myGroup);
            const mineFromClient = incoming.reports.filter((r) => String(r.group) === myGroup);
            record.reports = [...othersUnchanged, ...mineFromClient];
          }
        }
        ...
        // Список состава групп (эталонный список: имя/группа/номер/статус,
        // без привязки к месяцу) — редактирует и импортирует только админ.
        if (Array.isArray(incoming.groupRoster)) {
          if (!isAdmin) {
            return json({ error: "Недостаточно прав для изменения списка состава групп" }, 403);
          }
          record.groupRoster = incoming.groupRoster;
        }
        if (Array.isArray(incoming.groupRosterDraft)) {
          if (!isSuperadmin) {
            return json({ error: "Недостаточно прав для изменения черновика списка состава групп — доступно только главному админу" }, 403);
          }
          record.groupRosterDraft = incoming.groupRosterDraft;
        }
        if (incoming.rosterNotes && typeof incoming.rosterNotes === "object" && !Array.isArray(incoming.rosterNotes)) {
          const currentUserRecordForNotes = record.users.find((u) => u.email === auth.email);
          const hasNotesAccessForWrite = computeHasNotesContentAccess(currentUserRecordForNotes, isSuperadmin);
          if (!hasNotesAccessForWrite) {
            return json({ error: "Недостаточно прав для изменения заметок о возвещателях" }, 403);
          }
          record.rosterNotes = incoming.rosterNotes;
        }

        // Заявки на временный доступ к именам/заметкам. Обычный админ может
        // добавить/обновить только СВОЮ заявку — остальные записи в списке
        // трогать не может. Супер-админ управляет всем списком целиком
        // (например, удаляет запись при одобрении/отклонении заявки).
        if (Array.isArray(incoming.notesAccessRequests)) {
          if (isSuperadmin) {
            record.notesAccessRequests = incoming.notesAccessRequests;
          } else {
            const others = record.notesAccessRequests.filter((r) => r.email !== auth.email);
            const mine = incoming.notesAccessRequests.filter((r) => r.email === auth.email);
            record.notesAccessRequests = [...others, ...mine];
          }
        }
        ...
        if (Array.isArray(incoming.users)) {
          ...
          for (const incUser of incoming.users) {
            ...
            // Важно: для notesAccessExpiresAt нужно различать "поле вообще не
            // прислали" (оставить как было) от "прислали null" (явно снять
            // временный доступ) — оператор ?? этого не умеет, т.к. трактует
            // null и undefined одинаково. Поэтому здесь проверяем через "in".
            const hasExpiresAtField = Object.prototype.hasOwnProperty.call(incUser, "notesAccessExpiresAt");
            if (incUser.pass) {
              ...
              newUsersList.push({
                ...
                notesAccess: incUser.notesAccess ?? (existing ? existing.notesAccess : false),
                notesAccessExpiresAt: hasExpiresAtField ? incUser.notesAccessExpiresAt : (existing ? existing.notesAccessExpiresAt : null),
                notesContentAccess: incUser.notesContentAccess ?? (existing ? existing.notesContentAccess : false),
                pass,
              });
            } else if (existing) {
              newUsersList.push({
                ...
                notesAccess: incUser.notesAccess ?? existing.notesAccess,
                notesAccessExpiresAt: hasExpiresAtField ? incUser.notesAccessExpiresAt : existing.notesAccessExpiresAt,
                notesContentAccess: incUser.notesContentAccess ?? existing.notesContentAccess,
              });
            }
          }
        ...
        await writeBoard(env, record);
        const currentUserRecordFinal = record.users.find((u) => u.email === auth.email);
        const hasNotesAccessFinal = computeHasNotesAccess(currentUserRecordFinal, isSuperadmin);
        const hasNotesContentAccessFinal = computeHasNotesContentAccess(currentUserRecordFinal, isSuperadmin);
        return json({ data: { reports: record.reports, users: isSuperadmin ? stripPasswords(record.users) : [], ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: stripRosterNames(record.groupRoster, hasNotesAccessFinal), groupRosterDraft: isSuperadmin ? (record.groupRosterDraft || []) : [], rosterNotes: hasNotesContentAccessFinal ? (record.rosterNotes || {}) : {}, notesAccessRequests: isSuperadmin ? (record.notesAccessRequests || []) : (record.notesAccessRequests || []).filter((r) => r.email === auth.email) } });
      }

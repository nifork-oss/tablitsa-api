  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        // контакты), по personId. Тоже только админ/супер-админ — карточка
        // сейчас доступна только им.
        if (incoming.personMeta && typeof incoming.personMeta === "object" && !Array.isArray(incoming.personMeta)) {
          if (!isAdmin) {
            return json({ error: "Недостаточно прав для изменения личных данных карточки" }, 403);
          }

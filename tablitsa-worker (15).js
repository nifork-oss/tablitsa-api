...
function stripPasswords(users) {
  return (users || []).map((u) => ({ email: u.email, group: u.group, role: u.role, approved: u.approved === undefined ? true : !!u.approved, notesAccess: !!u.notesAccess }));
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (path === "/login" && request.method === "POST") {
        ...
        const hasNotesAccess = user.role === "superadmin" || !!user.notesAccess;
      ...
      if (path === "/appdata" && request.method === "GET") {
        ...
        const hasNotesAccess = isSuperadmin || !!(currentUserRecord && currentUserRecord.notesAccess);
        return json({
          data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: record.groupRoster || [], groupRosterDraft: record.groupRosterDraft || [], rosterNotes: hasNotesAccess ? (record.rosterNotes || {}) : {} },
          role: auth.role,
          email: auth.email,
        });
      }
      ...
      if (path === "/appdata" && request.method === "PUT") {
        ...
        if (Array.isArray(incoming.groupRosterDraft)) {
          if (!isAdmin) {
            return json({ error: "Недостаточно прав для изменения черновика списка состава групп" }, 403);
          }
        ...
        if (incoming.rosterNotes && typeof incoming.rosterNotes === "object" && !Array.isArray(incoming.rosterNotes)) {
          ...
          const hasNotesAccessForWrite = isSuperadmin || !!(currentUserRecordForNotes && currentUserRecordForNotes.notesAccess);
        ...
        const hasNotesAccessFinal = isSuperadmin || !!(currentUserRecordFinal && currentUserRecordFinal.notesAccess);
        return json({ data: { reports: record.reports, users: stripPasswords(record.users), ppNorms: record.ppNorms || {}, mergeLog: record.mergeLog || [], reportStatus: record.reportStatus || {}, personMeta: record.personMeta || {}, groupRoster: record.groupRoster || [], groupRosterDraft: record.groupRosterDraft || [], rosterNotes: hasNotesAccessFinal ? (record.rosterNotes || {}) : {} } });
      }
